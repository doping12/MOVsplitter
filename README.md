...
あと実行時間はめっちゃ長いです。-> 本編スキップ機能を入れたらだいぶ早くなりました。
あくまでも手動でやるのが面倒で投げ込んで放置しておきたいシチュエーションでの利用を想定しています。
精度も完璧ではないので若干1コマ2コマのloadingが入ります。
...
- 分割点を示すスコア関係
  ...
  - `score_timeline_refine.txt` (高精度境界探索ウィンドウのスコア時系列データ)
  - `check_boundary_plots/boundary_*_combined.png` (検知された境界前後を拡大したもの)
- 分割点のフレーム
  - `check_frames/*.jpg`
- `cut_table.csv` (分割リスト)
- `execution_timing.txt` (粗検知/高精度検知/分割の実時間)
- `used_config.yaml` または `used_config.json` (実行時に使った設定ファイルのコピー)
...
- `coarse_skip_after_loading_sec`: 粗検知でloadingを検出後に次の探索まで読み飛ばす秒数
- `coarse_skip_verify_window_sec`: skip発火前後を再サンプリングして粗検知境界を再確認する窓秒数
- `coarse_boundary_sample_fps`: 粗境界再探索で使うFPS
- `refine_sample_fps`: 高精度境界探索で使うFPS
- `refine_window_sec`: 境界の前後で再探索する秒数（±）
- `refine_parallel_workers`: 高精度境界探索の並列ワーカー数（0以下で自動）
...
- `template_param_dir`: テンプレート特徴量キャッシュの保存先ディレクトリ
...
- `ai_enabled`: AI補助判定を使うか
- `ai_confidence_threshold`: AI補助判定の採用しきい値
- `ai_model_path`: AI補助モデルのパス
- `ai_input_size`: AI補助モデル入力サイズ
